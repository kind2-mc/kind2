...
certificates are checked by cvc5, then turned into CPC proof objects by
...
certificates and actual proofs in the format of CPC. It will do so only for
...
CPC proofs production requires cvc5 (the binary can be specified with
``--cvc5_bin``), its Eunoia proof signatures, and the Ethos checker for the final
...
The script also downloads the cvc5 Eunoia signatures and
generates an easy-to-use bash script (``ethos-check.sh``) to
check CPC proofs generated by Kind 2:
...
CPC proofs. This can be done with the following invocation:
...
.. code-block:: none
   ...
   Final CPC proof written to add-two.lus.out/certificates.1/safety_proof.cpc
...
CPC proof is a formal proof that P is invariant in the system, using
sub-proofs of validity (unsatisfiability) returned by cvc5.

CPC signature
--------------

A proof system is formally defined in CPC through *signatures*, which contain
...
of type integer, the CPC encoding will make *y* and *z* respectively functions
...
our CPC encoding of a transition relation formula T[\ **x**\ , **x'**\ ] is
