...
collecting cvc5's own proofs and assembling them to form an overall Safety CPC proof that
...
certificates and actual proofs in the format of Safety CPC. It will do so only for
...
Safety CPC proofs production requires cvc5 (the binary can be specified 
with ``--cvc5_bin``), its Eunoia proof signatures, the safety proof 
signature (included  in the Kind 2 distribution), and the Ethos checker 
