...
let on_exit () =
  Format.printf "ind on exit1@.";
  (* Stop all timers *)
  Stat.ind_stop_timers ();
  Stat.smt_stop_timers ();
  Format.printf "ind on exit2@.";
...
let rec aux ts k solver =
	if k <> 1 then
		(
			S.assert_term solver (TransSys.trans_of_bound (Numeral.pred (Numeral.of_int k)) ts);
		  aux ts (k-1) solver
		)
