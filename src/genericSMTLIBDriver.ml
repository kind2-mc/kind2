...
let s_lambda = HString.mk_hstring "define-fun"
...
let rec expr_of_string_sexpr' bound_vars = function 
  ...
  (* A negative rational number as (/ (- n) d) *)
  | HStringSExpr.List
...
let lambda_of_string_sexpr bound_vars = function 

  (* (define-fun A (b) t) *)
  | HStringSExpr.List 
      [HStringSExpr.Atom s; HStringSExpr.Atom _; HStringSExpr.List v; t]
    when s == s_lambda -> 

    (* Get list of variables bound by the quantifier *)
    let vars = bound_vars_of_string_sexpr bound_vars [] v in
