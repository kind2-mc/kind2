...
type contract_summary = (LA.ident list) NodeId.NodeIdMap.t
...
let pp_print_node_summary ppf m =
  ...
  Lib.pp_print_list (fun ppf (i, b) ->
      Format.fprintf ppf "(%a->%a)"
        ...
        pp_print_val b
    ) ", " ppf (NodeId.NodeIdMap.bindings m)  
...
let empty_node_summary: node_summary = NodeId.NodeIdMap.empty

let empty_contract_summary: contract_summary = NodeId.NodeIdMap.empty
...
let union_dependency_analysis_data : dependency_analysis_data -> dependency_analysis_data -> dependency_analysis_data = 
  ...
  -> { graph_data = G.union g1 g2
    ...
    ; csummary = NodeId.NodeIdMap.union (fun _ _ v2 -> Some v2) cs1 cs2
    ; nsummary = NodeId.NodeIdMap.union (fun _ _ v2 -> Some v2) ns1 ns2
    ; nsummary2 = NodeId.NodeIdMap.union (fun _ _ v2 -> Some v2) n1 n2 }
...
let rec get_node_call_from_expr: LA.expr -> (LA.ident * Lib.position) list
  ...
  (* Node calls *)
  | LA.Call (pos, _, { name = i; }, es) -> (HString.concat2 node_prefix i, pos) :: List.flatten (List.map get_node_call_from_expr es)
...
= fun node_name -> function
  ...
  | LA.ContractCall (pos, { name = i; }, _, es, _) ->
     union_dependency_analysis_data
...
let mk_graph_node_decl: Lib.position -> LA.node_decl -> dependency_analysis_data
  = fun pos ({ name = i; }, _, _, _, ips, ops, locals, nitems, contract_opt) ->
...
let rec  mk_decl_map: LA.declaration option IMap.t -> LA.t -> ((LA.declaration option IMap.t), [> error]) result =
  ...
  | (LA.NodeDecl (span, ({ name = i; }, _, _, _, _, _, _, _, _)) as ndecl) :: decls
  | (LA.FuncDecl (span, ({ name = i; }, _, _, _, _, _, _, _, _)) as ndecl) :: decls ->
    let {LA.start_pos = pos} = span in
    let* m' = check_and_add m pos node_prefix i (Some ndecl) in
    mk_decl_map m' decls

  | LA.ContractNodeDecl (span, ({ name = i; }, _, _, _, _)) as cndecl :: decls ->
    let {LA.start_pos = pos} = span in
...
let rec vars_with_flattened_nodes: node_summary -> int -> LA.expr -> LA.SI.t
  ...
  | Call (_, _, i, es) ->
    ...
    let node_map = NodeId.NodeIdMap.find i m in
...
let rec mk_contract_eqn_map: LA.contract_node_equation option IMap.t -> LA.contract_node_equation list -> ((LA.contract_node_equation option IMap.t), [> error]) result
  ...
  | (LA.GhostVars (_, (GhostVarDec(_, [])), _)) :: eqns -> 
    mk_contract_eqn_map m eqns
  | (LA.ContractCall (pos, { name = i; }, _, _, _) as cc ) :: eqns -> 
...
let rec mk_graph_expr2: node_summary -> LA.expr -> (dependency_analysis_data list, [> error]) result
  ...
  | LA.Call (_, _, i, es) ->
     (match NodeId.NodeIdMap.find_opt i m with
...
let sort_and_check_contract_eqns: dependency_analysis_data
  ...
  Debug.parse "Sorting contract equations for %a" NI.pp_print_node_id i;
...
let get_contract_exports: contract_summary -> LA.contract_node_equation -> LA.ident list
  ...
  | LA.ContractCall (_, ({ name; } as cc), _, _, _) ->
     (match (NodeId.NodeIdMap.find_opt cc m) with
     | Some ids -> List.map (fun i -> HString.concat (HString.mk_hstring "::") [name;i]) ids
     | None -> failwith ("Undeclared contract " ^ (HString.string_of_hstring name) ^ ". Should not happen!"))  
 | _ -> []
...
let mk_contract_summary: contract_summary -> LA.contract_node_decl -> contract_summary
  ...
  NodeId.NodeIdMap.add i export_ids m 
...
let check_node_equations: dependency_analysis_data
  ...
  = fun ad (({ name; } as node_id, imported, opac, params, ips, ops, locals, items, contract_opt) as ndecl)->
  ...
  >> match contract_opt with
     ...
     | Some c ->
      let* (_, _, _, _, c') = sort_and_check_contract_eqns ad
        ((NI.mk_node_id (HString.concat (HString.mk_hstring "inline$") [contract_prefix;name])), params, ips, ops, c)
      in
...
let rec generate_summaries: dependency_analysis_data -> LA.t -> dependency_analysis_data
  ...
  | LA.FuncDecl (_, ndecl) :: decls ->
    ...
    generate_summaries {ad with 
      nsummary = NodeId.NodeIdMap.union (fun _ _ v2 -> Some v2) ad.nsummary ns;
      nsummary2 = NodeId.NodeIdMap.union (fun _ _ v2 -> Some v2) ad.nsummary2 ns_conn
    } decls
  ...
  | LA.ContractNodeDecl (_, cdecl) :: decls ->
     ...
     generate_summaries {ad with csummary = NodeId.NodeIdMap.union (fun _ _ v2 -> Some v2) ad.csummary cs } decls
