...
let pos_list_map : (Lib.position * A.eq_lhs) list A.NodeIdHashtbl.t = 
...
let rec update_if_position_info node_id ni = match ni with
  ...
  | Body (Equation (_, lhs, expr)) ->
    ...
    let if_info = match A.NodeIdHashtbl.find_opt pos_list_map node_id with
      | Some if_info2 -> (AH.pos_of_expr expr, lhs) :: if_info2
      | None -> [(AH.pos_of_expr expr, lhs)] 
    in
    A.NodeIdHashtbl.add pos_list_map node_id if_info;
  | _ -> ()
...
let desugar_node_decl ctx decl = match decl with
  | A.FuncDecl (s, (node_id, b, opac, nps, cctds, ctds, nlds, nis, co)) ->
    ...
    let gids = List.fold_left GI.union (GI.empty ()) gids in
    R.ok (A.FuncDecl (s, (node_id, b, opac, nps, cctds, ctds, new_decls @ nlds, nis, co)), A.NodeIdMap.singleton node_id gids)
  | A.NodeDecl (s, (node_id, b, opac, nps, cctds, ctds, nlds, nis, co)) ->
    let ctx = Chk.add_full_node_ctx ctx node_id nps cctds ctds nlds in
    let* nis = R.seq (List.map (desugar_node_item node_id ctx) nis) in
    let new_decls, nis, gids = split_and_flatten3 nis in
    let gids = List.fold_left GI.union (GI.empty ()) gids in
    R.ok (A.NodeDecl (s, (node_id, b, opac, nps, cctds, ctds, new_decls @ nlds, nis, co)), A.NodeIdMap.singleton node_id gids)
  | _ -> R.ok (decl, A.NodeIdMap.empty)
...
let desugar_if_blocks ctx sorted_node_contract_decls gids = 
  ...
  let gids2 = List.fold_left (A.NodeIdMap.merge GI.union_keys2) A.NodeIdMap.empty gids2 in
  let gids = A.NodeIdMap.merge GI.union_keys2 gids gids2 in
  R.ok (decls, gids)
