...
let desugar_node_decl ctx decl = match decl with
  | A.FuncDecl (s, (node_id, gen, b, opac, nps, cctds, ctds, nlds, nis, co)) ->
    ...
    let gids = List.fold_left GI.union (GI.empty ()) gids in
    R.ok (A.FuncDecl (s, (node_id, gen, b, opac, nps, cctds, ctds, new_decls @ nlds, nis, co)), NI.Map.singleton node_id gids)
  | A.NodeDecl (s, (node_id, gen, b, opac, nps, cctds, ctds, nlds, nis, co)) ->
    let ctx = Chk.add_full_node_ctx ctx node_id nps cctds ctds nlds in
    let* nis = R.seq (List.map (desugar_node_item node_id ctx) nis) in
    let new_decls, nis, gids = split_and_flatten3 nis in
    let gids = List.fold_left GI.union (GI.empty ()) gids in
    R.ok (A.NodeDecl (s, (node_id, gen, b, opac, nps, cctds, ctds, new_decls @ nlds, nis, co)), NI.Map.singleton node_id gids)
