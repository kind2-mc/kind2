...
(** Return a node hierarchy reduced to the cone of influence of
    ...
    [slice_to_abstraction m s] takes an association map [m] of scopes
    to a flag that is [true] if the node of that name is to be
    abstracted to its contract by omitting its implemenation. Return a
    node hierarchy based on [s], reduced to the cone of influence of
    the properties and contracts.
    ...
    variables occurring in contracts. If a node is sliced to its
    implementation, add the property state variables and all state
