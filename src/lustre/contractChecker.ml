...
let pp_print_realizability_result_pt
  ...
  let print_not_unknown_result tag =
    ...
    if String.length scope_str > inputs_tag_len && 
       String.sub scope_str 0 inputs_tag_len = LustreGenRefTypeImpNodes.gen_contract_inputs_tag then 
      Format.fprintf
        fmt
        ...
        (Stat.get_float Stat.analysis_time) 
            ...
            String.sub scope_str 0 contract_tag_len = LustreGenRefTypeImpNodes.gen_contract_tag then
      Format.fprintf
        fmt
