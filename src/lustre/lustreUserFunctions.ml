...
let valid_items set items =
  items |> List.for_all (function
    | A.Body (Equation (_, StructDef (_, [A.SingleIdent _]), rhs)) ->
      A.NodeIdSet.subset (AH.calls_of_expr rhs) set
...
let rec can_be_abstracted' ctx contracts (_, items) =
  items |> List.exists (function
    ...
    | ContractCall (_, id, _, _, _) -> (
        match A.NodeIdMap.find_opt id contracts with
        | None -> assert false
...
let is_inlinable (set: A.NodeIdSet.t) contracts ctx opac contract outputs locals items =
...
let inlinable_functions: Ctx.tc_context -> A.declaration list -> A.NodeIdSet.t 
= fun ctx decls ->
  List.fold_left (fun (set, contracts) dcl ->
    match dcl with
    | A.ContractNodeDecl (_, contract_node_decl) -> (
      let (id, _, _, _, _) = contract_node_decl in
      set, A.NodeIdMap.add id contract_node_decl contracts
    )
    ...
    | A.FuncDecl (_, (id, false, opac, [], _, outputs, locals, items, contract)) -> (
      if is_inlinable set contracts ctx opac contract outputs locals items then
        A.NodeIdSet.add id set, contracts
      else
        set, contracts
    )
    | _ -> set, contracts
  )
  (A.NodeIdSet.empty, A.NodeIdMap.empty)
  decls
  |> fst