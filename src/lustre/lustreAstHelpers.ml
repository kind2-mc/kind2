...
let rec expr_contains_type_ascription e =
  let opt_or a b = match a with Some _ -> a | None -> b in
  let r = expr_contains_type_ascription in
  match e with
  | TypeAscription _ as e -> Some e
  | Ident _ | EmptyMap (_, None) | EmptySet (_, None)
  | ModeRef (_, _) | Const _ -> None
  | EmptyMap (_, Some (kt, vt)) ->
    opt_or (fold_lustre_ty r None opt_or kt) (fold_lustre_ty r None opt_or vt)
  | EmptySet (_, Some ty) -> fold_lustre_ty r None opt_or ty
  | RecordProject (_, e, _) | UnaryOp (_, _, e)
  | ConvOp (_, _, e) | When (_, e, _) | Pre (_, e)
  | Extract (_, e, _, _) | StructUpdate (_, e, _, None)
    -> r e
  | Quantifier (_, _, tis, e) ->
    opt_or (List.fold_left (fun acc (_, _, ty) -> opt_or acc (fold_lustre_ty r None opt_or ty)) None tis) (r e)
  | BinaryOp (_, _, e1, e2) | CompOp (_, _, e1, e2) | StructUpdate (_, e1, _, Some e2)
  | ArrayConstr (_, e1, e2) | IndexAccess (_, e1, e2, _) | Arrow (_, e1, e2)
    -> opt_or (r e1) (r e2)
  | TernaryOp (_, _, e1, e2, e3)
    -> opt_or (opt_or (r e1) (r e2)) (r e3)
  | Call (_, tys, _, expr_list) ->
    opt_or (List.fold_left (fun acc ty -> opt_or acc (fold_lustre_ty r None opt_or ty)) None tys)
      (List.fold_left (fun acc x -> opt_or acc (r x)) None expr_list)
  | GroupExpr (_, _, expr_list)
    -> List.fold_left (fun acc x -> opt_or acc (r x)) None expr_list
  | RecordExpr (_, _, _, expr_list) | Merge (_, _, expr_list)
    -> List.fold_left (fun acc (_, e) -> opt_or acc (r e)) None expr_list
  | Activate (_, _, e1, e2, expr_list) ->
    opt_or (opt_or (r e1) (r e2)) (List.fold_left (fun acc x -> opt_or acc (r x)) None expr_list)
  | AnyOp (_, (_, _, ty), e)
  | ChooseOp (_, (_, _, ty), e) -> opt_or (fold_lustre_ty r None opt_or ty) (r e)
  | Condact (_, e1, e2, _, expr_list, expr_list2) ->
    opt_or (opt_or (r e1) (r e2))
      (opt_or (List.fold_left (fun acc x -> opt_or acc (r x)) None expr_list)
         (List.fold_left (fun acc x -> opt_or acc (r x)) None expr_list2))
  | RestartEvery (_, _, expr_list, e) ->
    opt_or (r e) (List.fold_left (fun acc x -> opt_or acc (r x)) None expr_list)
