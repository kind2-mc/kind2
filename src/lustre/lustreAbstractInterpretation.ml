...
type context = LA.lustre_type IMap.t LA.NodeNameMap.t
...
let union a b = LA.NodeNameMap.union
  (fun _ n1 n2 -> Some (IMap.union
    (fun _ _ i2 -> Some i2)
    n1 n2))
  a b

let get_type ctx node_name id = match LA.NodeNameMap.find_opt node_name ctx with
...
let add_type ctx node_name id ty =
  let update = LA.NodeNameMap.singleton node_name (IMap.singleton id ty) in
  union ctx update
...
and interpret_contract (node_id: LA.node_name) ctx ty_ctx contract =
...
and interpret_contract_eqn (node_id: LA.node_name) ctx ty_ctx = function
...
and interpret_node ty_ctx gids (id, _, _, ps, ins, outs, locals, items, contract) =
  (* Setup the typing context *)
  let ty_ctx = TC.add_io_node_ctx ty_ctx id ps ins outs in
  let ctx = LA.NodeNameMap.empty in
  ...
  let gids_node = LA.NodeNameMap.find id gids in
...
and check_global_const_subrange ty_ctx ctx pos_map =
  let ctx =
    match LA.NodeNameMap.find_opt dnode_id ctx with
    | None -> IMap.empty
    | Some ctx -> ctx
  in
