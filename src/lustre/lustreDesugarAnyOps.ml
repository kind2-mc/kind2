  ...
  | A.AnyOp (pos, (_, id, ty), expr1) -> 
    ...
    let generated_node = 
      if has_pre_arrow_or_node_call then
        A.NodeDecl (span, 
        (name, false, true, A.Opaque, ty_params, inputs,
...
fun ctx decls -> 
  ...
  List.fold_left (fun decls decl ->
    match decl with
    | A.NodeDecl (span, (id, gen, ext, opac, params, inputs, outputs, locals, items, contract)) ->
      ...
      decls @ gen_nodes @ gen_nodes2 @ [A.NodeDecl (span, (id, gen, ext, opac, params, inputs, outputs, locals, items, contract))]
    | A.FuncDecl (span, (id, gen, ext, opac, params, inputs, outputs, locals, items, contract)) ->
      let ctx = Chk.add_full_node_ctx ctx id params inputs outputs locals in
      let items, gen_nodes = List.map (desugar_node_item ctx id fun_ids) items |> List.split in 
      let contract, gen_nodes2 = desugar_contract ctx id fun_ids contract in
      let gen_nodes = List.flatten gen_nodes in
      decls @ gen_nodes @ gen_nodes2 @ [A.FuncDecl (span, (id, gen, ext, opac, params, inputs, outputs, locals, items, contract))]
