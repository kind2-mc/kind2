...
let rec initialize_pres init e = 
  let r = initialize_pres init in match e with 
  (* Only recurse into ITEs generated by if blocks. 
     We initialize each if block branch iff it is uninitialized. *)
  | A.TernaryOp (p, A.Ite true, e1, e2, e3) -> 
    A.TernaryOp (p, A.Ite true, r e1, r e2, r e3) 
  | e -> 
    if AH.expr_is_uninitialized e then
      Arrow (AH.pos_of_expr e, init, e) 
    else e

(** Parses an expression and replaces any ITE oracles with the 'fill'
    expression (which is stuttering, ie, 'pre variable').
*)
let rec fill_ib_oracles frame_pos node_id lhs fill e = 
  let r = fill_ib_oracles frame_pos node_id lhs fill in 
  match e with
...
(* Helper function to "push indices" further inside an expression, e.g. 
   ...
   This is a necessary normalization step for fill_ib_oracles, 
   ...
   But, if we push indices first, we convert equation 
     A[i] = (pre B)[i] to 
     A[i] = pre (B[i]), and fill_ib_oracles results in 
  ...
  | A.Body (Equation (pos, (StructDef(_, [SingleIdent(_, i)]) as lhs), rhs_expr)) -> 
    ...
    (match exprs with
      | Some (lhs, init, rhs_expr, pre_expr) ->     
        ...
        let rhs = 
          fill_ib_oracles f_pos node_id lhs 
            (A.Arrow (pos2, init, pre_expr)) rhs_expr
          |> initialize_pres init
        in
