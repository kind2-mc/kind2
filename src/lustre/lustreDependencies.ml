...
| A.NodeDecl ({A.start_pos=pos}, ((ident, _), _, _, _, _, _, _, _, _)) ->
  pos, ident |> HString.string_of_hstring |> I.mk_string_ident, NodeOrFun
| A.FuncDecl ({A.start_pos=pos}, ((ident, _), _,  _, _, _, _, _, _, _)) ->
  pos, ident |> HString.string_of_hstring |> I.mk_string_ident, NodeOrFun

| A.ContractNodeDecl ({A.start_pos=pos}, ((ident, _), _, _, _, _)) ->
...
let insert_decl decl (f_type, f_ident) decls =
  ...
  let has_ident = match f_type with
    | NodeOrFun -> (
      function
      | A.NodeDecl (_, ((i, _), _, _, _, _, _, _, _, _)) -> i = ident
      | A.FuncDecl (_, ((i, _), _, _, _, _, _, _, _, _)) -> i = ident
      | _ -> false
    )
    | Type -> (
      function
    ...
    | Contract -> (
      function
      | A.ContractNodeDecl (_, ((i, _), _, _, _, _)) -> i = ident
      | _ -> false
    )
    | Const -> (
      function
