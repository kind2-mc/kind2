...
let pp_print_generated_identifiers ppf gids =
  ...
  let pp_print_oracle ppf (id, ty, e) = Format.fprintf ppf "(%a, %a, %a)"
    ...
    LustreAst.pp_print_expr e
  in
  let pp_print_call = (fun ppf (pos, output, cond, restart, ident, args, defaults, inlined) ->
    Format.fprintf ppf 
  ...
  List.fold_left
    (fun acc decl ->
     ...
     | A.FuncDecl (_, nd) ->
       ...
       if A.SI.mem id inlinable_funcs then
         StringMap.add id nd acc
       else
         acc
     | _ -> acc
    )
...
and normalize_ty info node_id map id ty = 
  ...
  | ArrayType _ | EnumType _ | TArr _ | Map _ | BitVector _ -> ty, empty (), []
