...
type info = {
  ...
  node_is_input_const : (bool list) NI.NodeIdMap.t;
  contract_calls_info : LustreAst.contract_node_decl NI.NodeIdMap.t;
  contract_scope : (Lib.position * NI.node_id) list;
  contract_ref : HString.t;
  interpretation : HString.t StringMap.t;
  local_group_projection : int;
  inlinable_funcs : LustreAst.node_decl NI.NodeIdMap.t;
}
...
let pp_print_generated_identifiers ppf gids =
  ...
  let pp_print_contract_call ppf (ref, pos, scope, decl) = Format.fprintf ppf "%a := (%a, %a): %a"
    ...
    pp_print_position pos
    (pp_print_list (pp_print_pair Lib.pp_print_position NI.pp_print_node_id ":") "::") scope
...
let compute_node_input_constant_mask decls =
  ...
  | A.NodeDecl (_, (id, _, _, _, inputs, _, _, _, _)) ->
    ...
    NI.NodeIdMap.add id is_consts map
  | FuncDecl (_, (id, _, _, _, inputs, _, _, _, _)) ->
    let is_consts = List.map (fun (_, _, _, _, c) -> c) inputs in
    NI.NodeIdMap.add id is_consts map
  | _ -> map
  in List.fold_left over_decl NI.NodeIdMap.empty decls

let collect_contract_node_decls decls =
  let over_decl map = function
  | A.ContractNodeDecl (_, (id, params, inputs, outputs, equations)) ->
    NI.NodeIdMap.add id (id, params, inputs, outputs, equations) map
  | _ -> map
  in List.fold_left over_decl NI.NodeIdMap.empty decls
...
let get_inline_func_expr inlinable_funcs name args =
  let (_, _, _, _, inputs, _, _, items, _) : A.node_decl =
    match NI.NodeIdMap.find_opt name inlinable_funcs with
    | Some nd -> nd
    | None -> assert false
  in
  let var_map =
...
let mk_fresh_call ?(inlined=false) info id map pos cond restart ty_args args defaults =
  let called_node = NI.NodeIdMap.find id map in 
  ...
  List.fold_left
    (fun acc decl ->
     ...
     | A.FuncDecl (_, nd) ->
       ...
       if NI.NodeIdSet.mem id inlinable_funcs then
         NI.NodeIdMap.add id nd acc
       else
         acc
     | _ -> acc
    )
...
*)
  ...
  match NI.NodeIdMap.find_opt node_id gids_map with
  | None -> empty(), []
  | Some gids -> (
...
and normalize_declaration info map = function
  ...
  | ContractNodeDecl (_, (id, ps, ips, ops, _)) ->
    ...
    let map = NI.NodeIdMap.add id ngids map in
    None, map, warnings
  | decl -> Some decl, map, []
...
and normalize_node info map
  ...
  let map = NI.NodeIdMap.add node_id (union old_gids new_gids) map in
...
(*         let new_name = info.contract_ref ^ "_contract_" ^ name in
      ...
      | ContractCall (pos, name, ty_args, inputs, outputs) ->
        ...
        in
        let called_node = NI.NodeIdMap.find name info.contract_calls_info in
...
and normalize_expr ?guard info node_id map =
  ...
  | Call (pos, ty_args, id, args) ->
    let is_inlinable = NI.NodeIdMap.mem id info.inlinable_funcs in
    let info, vmap, gids0 =
    ...
    let handle_call inlined args =
      let flags = NI.NodeIdMap.find id info.node_is_input_const in
  ...
  | Condact (pos, cond, restart, id, args, defaults) ->
    let flags = NI.NodeIdMap.find (NI.mk_node_id id) info.node_is_input_const in
