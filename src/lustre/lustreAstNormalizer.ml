...
type info = {
  ...
  node_is_input_const : (bool list) A.NodeIdMap.t;
  contract_calls_info : LustreAst.contract_node_decl A.NodeIdMap.t;
  contract_scope : (Lib.position * A.node_id) list;
  contract_ref : HString.t;
  interpretation : HString.t StringMap.t;
  local_group_projection : int;
  inlinable_funcs : LustreAst.node_decl A.NodeIdMap.t;
}
...
let pp_print_generated_identifiers ppf gids =
  ...
  let pp_print_contract_call ppf (ref, pos, scope, decl) = Format.fprintf ppf "%a := (%a, %a): %a"
    ...
    pp_print_position pos
    (pp_print_list (pp_print_pair Lib.pp_print_position A.pp_print_node_id ":") "::") scope
...
let compute_node_input_constant_mask decls =
  ...
  | A.NodeDecl (_, (id, _, _, _, inputs, _, _, _, _)) ->
    ...
    A.NodeIdMap.add id is_consts map
  | FuncDecl (_, (id, _, _, _, inputs, _, _, _, _)) ->
    let is_consts = List.map (fun (_, _, _, _, c) -> c) inputs in
    A.NodeIdMap.add id is_consts map
  | _ -> map
  in List.fold_left over_decl A.NodeIdMap.empty decls

let collect_contract_node_decls decls =
  let over_decl map = function
  | A.ContractNodeDecl (_, (id, params, inputs, outputs, equations)) ->
    A.NodeIdMap.add id (id, params, inputs, outputs, equations) map
  | _ -> map
  in List.fold_left over_decl A.NodeIdMap.empty decls
...
let get_inline_func_expr inlinable_funcs name args =
  let (_, _, _, _, inputs, _, _, items, _) : A.node_decl =
    match A.NodeIdMap.find_opt name inlinable_funcs with
    | Some nd -> nd
    | None -> assert false
  in
  let var_map =
...
let mk_fresh_call ?(inlined=false) info id map pos cond restart ty_args args defaults =
  let called_node = A.NodeIdMap.find id map in 
  ...
  List.fold_left
    (fun acc decl ->
     ...
     | A.FuncDecl (_, nd) ->
       ...
       if A.NodeIdSet.mem id inlinable_funcs then
         A.NodeIdMap.add id nd acc
       else
         acc
     | _ -> acc
    )
...
*)
  ...
  match A.NodeIdMap.find_opt node_id gids_map with
  | None -> empty(), []
  | Some gids -> (
...
and normalize_declaration info map = function
  ...
  | ContractNodeDecl (_, (id, ps, ips, ops, _)) ->
    ...
    let map = A.NodeIdMap.add id ngids map in
    None, map, warnings
  | decl -> Some decl, map, []
...
and normalize_node info map
  ...
  let map = A.NodeIdMap.add node_id (union old_gids new_gids) map in
...
(*         let new_name = info.contract_ref ^ "_contract_" ^ name in
      ...
      | ContractCall (pos, name, ty_args, inputs, outputs) ->
        ...
        in
        let called_node = A.NodeIdMap.find name info.contract_calls_info in
        ...
        ContractCall (pos, (cref, A.NodeTagSet.empty), ty_args, inputs, outputs), gids, warnings, interp
      | GhostConst decl ->
...
and normalize_expr ?guard info node_id map =
  ...
  | Call (pos, ty_args, id, args) ->
    let is_inlinable = A.NodeIdMap.mem id info.inlinable_funcs in
    let info, vmap, gids0 =
    ...
    let handle_call inlined args =
      let flags = A.NodeIdMap.find id info.node_is_input_const in
  ...
  | Condact (pos, cond, restart, id, args, defaults) ->
    let flags = A.NodeIdMap.find (id, A.NodeTagSet.empty) info.node_is_input_const in
    ...
    let nexpr, gids5 = mk_fresh_call info (id, A.NodeTagSet.empty) map pos ncond nrestart [] nargs (Some ndefaults) in
  ...
  | RestartEvery (pos, id, args, restart) ->
    ...
    in let nargs, gids2, warnings2 = normalize_list
      ...
      (combine_args_with_const info args flags)
    in
    let nexpr, gids3 = mk_fresh_call info (id, A.NodeTagSet.empty) map pos cond nrestart [] nargs None in
    ...
    let normalize' info map ?guard = function
      | clock_value, A.Activate (pos, id, cond, restart, args) ->
        ...
        let nargs, gids3, warnings3 = normalize_list
          ...
          (combine_args_with_const info args flags)
        in
        let nexpr, gids4 = mk_fresh_call info (id, A.NodeTagSet.empty) map pos ncond nrestart [] nargs None in
