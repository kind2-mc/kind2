...
let rec gen_poly_decl: Ctx.tc_context -> GI.t NI.NodeIdMap.t -> NI.node_id option -> (A.declaration * A.lustre_type list list) NI.NodeIdMap.t ->
                   NI.node_id -> A.lustre_type list -> Ctx.tc_context * GI.t NI.NodeIdMap.t * NI.node_id *  A.declaration list * (A.declaration * A.lustre_type list list) NI.NodeIdMap.t 
= fun ctx gids caller_nname node_decls_map ({ name; node_type; } as nname) ty_args ->
  (* Check for pre-existing instantiation of the node before compiling a new one *)
  let decl, tyss = NI.NodeIdMap.find nname node_decls_map in 
  let find_decl tys = 
  ...
  | Some j -> 
    let pnname = { NI.name; node_type; monomorphization=Some j } in
  ...
  | None ->
    let span, is_function, is_contract, ext, opac, ips, ops, locs, nis, c =
      match NI.NodeIdMap.find nname node_decls_map with
    ...
    let pnname = { NI.name; node_type; monomorphization = Some (List.length tyss); } in
    (* Remember new instantiation *)
    let node_decls_map = NI.NodeIdMap.add nname (decl, tyss @ [ty_args]) node_decls_map in
    let ctx, called_decl = 
      if is_function then 
    ...
    let ctx, gids, decls, node_decls_map = match NI.NodeIdMap.find_opt nname gids with
    | None -> 
      (* let gids = NI.NodeIdMap.add nname { (GI.empty ()) with 
    ...
    | Some polymorphic_gids -> 
      ...
      let gids = NI.NodeIdMap.add pnname monomorphized_gids gids in 
...
and gen_poly_decls_ty: Ctx.tc_context -> GI.t NI.NodeIdMap.t -> NI.node_id option -> (A.declaration * A.lustre_type list list) NI.NodeIdMap.t ->
                           A.lustre_type -> Ctx.tc_context * GI.t NI.NodeIdMap.t * A.lustre_type * A.declaration list * (A.declaration * A.lustre_type list list) NI.NodeIdMap.t
...
and gen_poly_decls_expr: Ctx.tc_context -> GI.t NI.NodeIdMap.t -> NI.node_id option -> (A.declaration * A.lustre_type list list) NI.NodeIdMap.t ->
                             A.expr -> Ctx.tc_context * GI.t NI.NodeIdMap.t * A.expr *  A.declaration list * (A.declaration * A.lustre_type list list) NI.NodeIdMap.t
...
let instantiate_polymorphic_nodes: Ctx.tc_context -> GI.t NI.NodeIdMap.t -> A.declaration list -> Ctx.tc_context * GI.t NI.NodeIdMap.t  * A.declaration list 
= fun ctx gids decls -> 
  ...
  | (ContractNodeDecl (_, (id, _, _, _, _)) as decl) -> NI.NodeIdMap.add id (decl, []) acc
  | TypeDecl _ | ConstDecl _ | NodeParamInst _ -> acc
  ) NI.NodeIdMap.empty decls 
  in
