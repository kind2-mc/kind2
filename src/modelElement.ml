...
let print_mcs_counterexample in_sys param sys typ fmt (prop, cex) =
  try
    ...
    then
      match typ with
      | `PT ->
        KEvent.pp_print_counterexample_pt L_warn in_sys param sys (Some prop) true ((TransSys.property_of_name sys prop).prop_kind) fmt cex
      | `XML ->
        KEvent.pp_print_counterexample_xml in_sys param sys (Some prop) true ((TransSys.property_of_name sys prop).prop_kind) fmt cex
      | `JSON ->
        KEvent.pp_print_counterexample_json in_sys param sys (Some prop) true ((TransSys.property_of_name sys prop).prop_kind) fmt cex
  with _ -> ()
