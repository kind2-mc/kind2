...
(* (\** Sigature of a solver *\) *)
(* module type Scaca = sig *)

(*   (\** Type of solver instances *\) *)
(*   type t *)
...
(*                                       The executable must be the first element *)
(*                                       in the array, each subsequent string is *)
(*                                       an argument that is passed to *)
(*                                       [Unix.open_process] as is. *\) *)
(*     } *)

(*   (\** {1 Managing solver instances} *\) *)

(*   (\** [create_instance l id] creates a new instance of the SMT solver with a *)
(*       unique identifier [id], initialized to the logic [l] and produces *)
...
(*   val declare_fun : *)
(*     t -> string -> SMTExpr.sort list -> SMTExpr.sort -> decl_response *)

(*   (\** Define a new function symbol as an abbreviation for an expression *\) *)
(*   val define_fun : *)
(*     t -> string -> SMTExpr.var list -> SMTExpr.sort -> SMTExpr.t -> *)
(*     decl_response *)

(*   (\** {1 Commands} *\) *)

(*   (\** Assert the expression *\) *)
(*   val assert_expr : t -> SMTExpr.t -> decl_response *)

(*   (\** Push a number of empty assertion sets to the stack *\) *)
(*   val push : t -> int -> decl_response  *)

(*   (\** Pop a number of assertion sets from the stack *\) *)
(*   val pop : t -> int -> decl_response  *)
...
(*   val check_sat : ?timeout:int -> t -> check_sat_response *)

(*   (\** Check satisfiability of the asserted expressions assuming the input *)
(*       literals. *\) *)
(*   val check_sat_assuming : t -> SMTExpr.t list -> check_sat_response *)
...
(*   val get_value : t -> SMTExpr.t list -> get_value_response *)

(*   (\** Get an unsatisfiable core of named expressions *\) *)
(*   val get_unsat_core : t -> get_unsat_core_response *)
...
(*   val execute_custom_check_sat_command : string -> t -> check_sat_response *)
