...
let main input_file input_sys _ trans_sys =
  ...
  KEvent.set_module `CMonitor;

  let input_scope = TransSys.scope_of_trans_sys trans_sys @
                    LustreIdent.user_scope in
      input_scope |> List.iter (fun l -> (Printf.printf "Input scope: %s\n" l)) ;
  ...
  let inputs =   
    if input_file = "" then []
      ...
      try InputParser.read_file ~only_inputs:false input_scope input_file 
      with Sys_error e -> 
        (* Output warning *)
        KEvent.log L_warn "@[<v>Error reading interpreter input file.@,%s@]" e;
        raise (Failure "main")
  in
(* Use only contract in transition system *)
  ...
  List.iter
    ...
    inputs;
  (* Printf.printf "Calculating steps\n" ; *)
  ...
  KEvent.log_uncond "Interpreter running up to k=%d" steps;
  (* Determine logic for the SMT solver *)
  let logic = TransSys.get_logic trans_sys in
  (* Need to also assert that the inputs are a value at a particular timestep, 
  but now tih CM, need also the outputs to be asserted*)
  ...
  if (SMTSolver.check_sat solver) then
    (
      ...
      let y = TransSys.get_state_var_bounds trans_sys in
      let x = TransSys.vars_of_bounds trans_sys
                  Numeral.zero (Numeral.of_int steps) in
      let st_vars = TransSys.state_vars trans_sys in
      let model = SMTSolver.get_var_values solver y x in
      let path = 
      ...
      let p = Model.path_to_list path in    
