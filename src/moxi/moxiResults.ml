...
let pp_const_decl ppf const_decl_path =
  ...
  | _ -> failwith (Format.asprintf "Recieved unexpected model value. Unable to construct counter example.\n
                                    ...
                                    StateVar.pp_print_state_var svar (Lib.pp_print_list Model.pp_print_value ", ") svar_value_path) 

let pp_const_decls: TransSys.t -> Format.formatter -> (StateVar.t * Model.value list) list -> unit 
= fun trans_sys ppf svar_path -> 
  let const_svars = List.map (fun svar -> svar, List.assoc svar svar_path) (TransSys.global_const_state_vars trans_sys) in
  Format.fprintf ppf "%a" (Lib.pp_print_list pp_const_decl "@,") const_svars
...
let pp_print_prop_model: TransSys.t -> 'b -> Property.t -> unit 
= fun trans_sys _ prop ->
  let prop_name = prop.Property.prop_name in
  match Property.get_prop_status prop with
  | PropFalse cex -> Format.printf 
                    "@[<hv 2>:model@ (@[<v>%a@]@])@,"
