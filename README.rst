...
invariant (see `2_input/1_lustre <doc/usr/source/2_input/1_lustre.rst>`_), and
...
Kind 2 can output its results in XML format (see `3_output/2_xml <doc/usr/source/3_output/2_xml.rst>`_).
...
The following command-line options control its operation (run ``kind2 --help`` for a full list). See `1_techniques/1_techniques <doc/usr/source/1_techniques/1_techniques.rst>`_ for configuration examples and more details on each technique.
