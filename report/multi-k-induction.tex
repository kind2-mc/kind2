...
\begin{figure}[tp]
  ...
  \begin{algorithmic}[1]
    ...
    \State \Let $\mathcal{P} = \mathcal{P} \cup \bigcup \{ D \mid (l, D) \in \mathcal{D}_\mathrm{invalid} \} \setminus \mathcal{P}_\mathrm{invalid}$ \In
    ...
    \State \Let {$\Phi_2 = \mathcal{P}_\mathrm{valid}[\vec{x_0},\dotsc,\vec{x_{k+1}}] \land \land \mathcal{T}[\vec{x_0},\dotsc,\vec{x_{k+1}}] $} \In

    % k-induction check 
    \If {$\Phi_1 \land \Phi_2 \lent \mathcal{P}[\vec{x_{k+1}}]$} 

      % Can there be counterexamples of length up to k? 
      \If {$k_\textproc{bmc} \geq k$}
